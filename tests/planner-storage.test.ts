...
test('플래너 세션 저장값은 허용된 범위만 복원한다', () => {
  ...
  assert.deepEqual(parsePlannerGroupSettings('{"memberCount":99}'), { isSolo: false, memberCount: 1 })
  assert.deepEqual(parsePlannerGroupSettings('invalid'), { isSolo: false, memberCount: 1 })
