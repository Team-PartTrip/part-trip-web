...
test('세계지도 API와 국가 획득 화면이 실제 서버 계약을 사용한다', () => {
  ...
  const flow = read('/src/widgets/profile-insights/model/useProfileInsightFlow.ts')
  const views = read('/src/widgets/profile-insights/ui/ProfileInsightModeViews.tsx')
  const map = read('/src/shared/ui/world-map/index.tsx')
  const asset = read('/src/shared/assets/world-map.svg')
  const profile = `${page}\n${views}`
  ...
  assert.match(flow, /useAcquireCountryMutation/)
  assert.match(flow, /acquireCountryMutation\.mutateAsync\(\{\s*tripId/)
  assert.match(flow, /isNew/)
  assert.match(profile, /<WorldMap/)
  assert.match(map, /world-map\.svg\?raw/)
  assert.match(map, /path\[id=/)
  assert.match(map, /data-id/)
  assert.doesNotMatch(profile, /getCountryMarkerPosition|MapMarker|figmaWorldMap/)
  assert.match(asset, /id="JP"[\s\S]*data-id="JP"/)
  assert.match(asset, /id="KR"[\s\S]*data-id="KR"/)
  assert.match(asset, /id="RU"/)
  assert.doesNotMatch(asset, /fill:#(?:2478c5|1a6ebf)/i)
})
