    ...
    <AppShell>
      <S.Page>
        <S.Header>
          ...
          <S.BackButton type="button" onClick={() => navigate({ to: paths.record })}>목록으로</S.BackButton>
        </S.Header>
        <S.Body>
          ...
          <S.List>
            ...
            {trips.map((trip) => <button key={trip.tripId ?? trip.title} type="button" onClick={() => trip.tripId && navigate({ params: { recordId: String(trip.tripId) }, to: '/record/$recordId' })}><strong>{trip.title || '여행 기록'}</strong><span>{trip.cityName || trip.countryName || '여행지'}</span></button>)}
          </S.List>
        </S.Body>
      </S.Page>
    </AppShell>
