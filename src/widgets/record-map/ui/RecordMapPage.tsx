    ...
    <AppShell>
      <S.Page>
        <S.Header>
          ...
          <S.BackButton type="button" onClick={() => navigate({ to: paths.record as never })}>목록으로</S.BackButton>
        </S.Header>
        <S.Body>
          ...
          <S.List>
            ...
            {trips.map((trip) => <button key={trip.tripId ?? trip.title} type="button" onClick={() => trip.tripId && navigate({ to: `/record/${trip.tripId}` as never })}><strong>{trip.title || '여행 기록'}</strong><span>{trip.cityName || trip.countryName || '여행지'}</span></button>)}
          </S.List>
        </S.Body>
      </S.Page>
    </AppShell>
