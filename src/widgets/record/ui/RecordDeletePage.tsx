...
export function RecordDeletePage() {
  ...
  const selectAll = () => setSelected(photoIds)
  ...
  return (
    <AppShell>
      <S.Page>
        ...
        {!isLoading && !hasError ? <><S.Toolbar><strong>{selected.length}개 선택됨</strong><div><button type="button" onClick={selectAll}>전체 선택</button><button type="button" onClick={clearSelection}>선택 해제</button></div></S.Toolbar><S.DeleteLayout><S.DeleteList><h2>{record?.title || '여행 기록'} · 사진 {photos.length}장</h2><S.PhotoGrid>{photos.map((photo, index) => { const entryId = photo.entryId as number; return <S.PhotoButton key={entryId} type="button" $selected={selected.includes(entryId)} onClick={() => setSelected((current) => current.includes(entryId) ? current.filter((item) => item !== entryId) : [...current, entryId])}><img src={photo.imageUrl} alt={`여행 사진 ${index + 1}`} /><span aria-hidden="true">{selected.includes(entryId) ? '✓' : ''}</span></S.PhotoButton> })}</S.PhotoGrid></S.DeleteList><S.DeletePanel><h2>삭제 확인</h2><S.Warning><strong>! 삭제하면 되돌릴 수 없어요</strong><span>삭제한 사진 대신 기본 대체 이미지가 표시됩니다.</span></S.Warning><S.DeleteSummary><strong>선택한 사진 {selected.length}장</strong><span>여행 기간 중인 사진만 삭제할 수 있습니다.</span></S.DeleteSummary><S.DeleteActions><S.DeleteButton type="button" disabled={selected.length === 0 || deleteMutation.isPending} onClick={() => void handleDelete()}>{deleteMutation.isPending ? '삭제 중' : `${selected.length}개 삭제하기`}</S.DeleteButton><button type="button" onClick={() => navigate({ to: paths.record })}>취소</button></S.DeleteActions></S.DeletePanel></S.DeleteLayout></> : null}
      </S.Page>
    </AppShell>
  )
}
