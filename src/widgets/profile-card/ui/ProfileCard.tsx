...
export function ProfileCard({ profile }: ProfileCardProps) {
  ...
  const badges = [badge1Url, badge2Url, badge1Url, badge2Url, badge3Url, badge4Url, badge3Url, badge4Url, badge5Url, badge6Url, badge5Url, badge6Url]
    ...
    <S.Card>
      ...
      <S.Body>
        <S.BadgePanel>
          ...
          <S.BadgeGrid>{badges.map((badge, index) => <img src={badge} alt={`여행 배지 ${index + 1}`} key={`${badge}-${index}`} />)}</S.BadgeGrid>
        </S.BadgePanel>
        <S.TypePanel>
          <h2>여행 타입 상세</h2>
