...
export function RecordCalendarPage() {
  ...
  const eventByDay = new Map(festivals.map((festival) => [Number(festival.startDate?.slice(-2)), festival]))
  const isLoading = isPlanLoading || isFestivalsLoading

  return (
    <AppShell>
      <S.Page>
        <S.Header><S.Title>축제 · 이벤트</S.Title><S.Subtitle>여행 기간 내 해당 나라의 축제 / 공연 / 마켓 정보를 확인하세요.</S.Subtitle></S.Header>
        <S.CalendarLayout>
          <S.CalendarCard><S.MonthBar><div><h2>{month.getFullYear()}년 {month.getMonth() + 1}월</h2><p>{plan?.cityName || plan?.countryName || '여행지'} · 여행 기간 {plan ? `${formatDate(plan.startDate)} – ${formatDate(plan.endDate)}` : '미설정'}</p></div><span><button type="button" aria-label="이전 달" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))}>‹</button><button type="button" aria-label="다음 달" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))}>›</button></span></S.MonthBar><S.Weekdays>{weekdays.map((day) => <span key={day}>{day}</span>)}</S.Weekdays><S.CalendarGrid>{cells.map((day, index) => { const event = day ? eventByDay.get(day) : undefined; const inTrip = day != null && plan?.startDate && plan?.endDate && `${month.getFullYear()}-${String(month.getMonth() + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}` >= plan.startDate && `${month.getFullYear()}-${String(month.getMonth() + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}` <= plan.endDate; return <S.Cell key={`${day}-${index}`} $empty={day === null} $inTrip={Boolean(inTrip)}>{day ? <><strong>{day}</strong>{event ? <S.EventLabel>{event.category || '행사'}</S.EventLabel> : null}</> : null}</S.Cell> })}</S.CalendarGrid>{isLoading ? <S.Note>이벤트를 불러오는 중입니다.</S.Note> : null}{!isLoading && festivals.length === 0 ? <S.Note>연결된 이벤트 데이터가 없습니다.</S.Note> : null}</S.CalendarCard>
          <S.FestivalList><h2>이달의 축제 {festivals.length}건</h2>{festivals.map((event) => <S.FestivalRow key={`${event.festivalId ?? event.title}-${event.startDate}`}><div><strong>{event.title || '이름 없는 이벤트'}</strong><span>{eventDateLabel(event)}{event.location ? ` · ${event.location}` : ''}</span></div><small>{event.category || '행사'}</small></S.FestivalRow>)}{!isLoading && festivals.length === 0 ? <S.Note>표시할 행사가 없습니다.</S.Note> : null}<button type="button" onClick={() => navigate({ to: paths.record })}>여행 기록으로 돌아가기</button></S.FestivalList>
        </S.CalendarLayout>
      </S.Page>
    </AppShell>
  )
