...
function NotificationFlow({ mode }: { mode: Mode }) {
  ...
  return (
    <AppShell>
      <S.Page>
        <S.Header><div><S.Title>{mode === 'settings' ? '알림 설정' : mode === 'detail' ? '알림 상세' : '알림'}</S.Title><S.Subtitle>{mode === 'settings' ? '받고 싶은 알림을 선택하세요.' : '여행과 기록에 대한 소식을 확인하세요.'}</S.Subtitle></div>{mode !== 'list' ? <S.BackButton type="button" onClick={() => navigate({ to: paths.notifications as never })}>알림 목록</S.BackButton> : null}</S.Header>
        ...
        {mode === 'detail' ? <S.Detail><S.StatusDot /><h2>알림 상세 화면</h2><p>알림 API 계약이 없어 실제 알림 내용과 읽음 처리를 아직 조회할 수 없습니다.</p><S.ActionRow><PartTripButton type="button" disabled>읽음 처리</PartTripButton><PartTripButton type="button" $variant="secondary" onClick={() => navigate({ to: paths.notifications as never })}>목록으로</PartTripButton></S.ActionRow></S.Detail> : null}
