      ...
      <S.Page $wide={step === 'destination' || step === 'place'}>
        ...
        {step === 'list' ? (
          <>
            ...
            <S.PlannerListLayout>
              <S.PlanListPanel>
                {availablePlanners.map((planner, index) => <S.PlanRow key={planner.plannerId ?? index} type="button" $state={plannerStatusKey(planner.status)} onClick={() => void handleSelectPlanner(planner.plannerId)}><S.PlanContent><S.PlanDetails><strong>{planner.title || `${planner.cityName || planner.countryName || '여행'} 여행`}</strong><span>{dateRange(planner.startDate, planner.endDate)}</span><S.PlanStatusRow><S.PlanStatus $state={plannerStatusKey(planner.status)}>{plannerStatusLabel(planner.status)}</S.PlanStatus><S.PlanParticipation>{planner.joinedMemberCount ?? 0}/{planner.memberCount ?? 0}명 참여</S.PlanParticipation></S.PlanStatusRow></S.PlanDetails><S.PlanAside><S.RowArrow aria-hidden="true">›</S.RowArrow></S.PlanAside></S.PlanContent></S.PlanRow>)}
        ...
        {step === 'lineup' ? <S.TwoColumn><S.StepCard><S.SectionTitle>선택한 장소 {selectedPlaces.length}개</S.SectionTitle><S.SelectedPlaces>{selectedPlaces.map(({ index, item }) => <S.SelectedPlaceRow key={`${item.placeName}-${index}`}><S.PlaceMarker /><S.PlaceDetails><strong>{item.placeName || '장소'}</strong><span>{plan?.cityName || plan?.countryName || '여행지'} · {item.category || '장소'}</span></S.PlaceDetails><small>{index + 1} votes</small></S.SelectedPlaceRow>)}</S.SelectedPlaces>{selectedPlaces.length === 0 ? <S.Empty>투표 화면에서 장소를 선택하세요.</S.Empty> : null}</S.StepCard><S.NextPanel><S.SectionTitle>다음 단계</S.SectionTitle><p>선택한 장소를 바탕으로 멤버 투표를 시작합니다.</p><label>선택 방식</label><S.SegmentRow><S.SegmentButton type="button" $active={lineupChoice !== null} onClick={() => { const first = selectedPlaces[0]?.index; if (first != null) { setLineupChoice(first); setSelected([first]) } }}>직접 선택</S.SegmentButton><S.SegmentButton type="button" $active={lineupChoice === null} onClick={() => void handleRandomLineup()}>랜덤 뽑기</S.SegmentButton></S.SegmentRow><S.Notice>고르기 어렵다면?<br />남은 장소 중에서 하나를 서버에서 랜덤으로 뽑아 드려요. 소수 인원일 때 사용하세요.</S.Notice><S.ActionRow><PartTripButton type="button" disabled={isSavingCandidates} onClick={() => void handleSaveCandidates()}>투표 진행</PartTripButton><PartTripButton type="button" $variant="secondary" onClick={() => flowNavigate({ to: paths.plannerExplore })}>장소 더 찾기</PartTripButton></S.ActionRow></S.NextPanel></S.TwoColumn> : null}
