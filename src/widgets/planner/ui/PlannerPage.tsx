...
import { getCountries, getDday, getTourPlace, saveTravelPlan, type CountryInfoResponseDto, type DdayResponseDto, type TourPlaceResponseDto } from '@/shared/api'
...
function usePlannerData() {
  const [plan, setPlan] = useState<DdayResponseDto>()
  const [places, setPlaces] = useState<TourPlaceResponseDto[]>([])
  ...
  useEffect(() => {
    let isMounted = true
    void Promise.all([getDday().catch(() => undefined), getCountries().catch(() => [])])
      .then(([nextPlan, nextCountries]) => {
        if (!isMounted) return
        setPlan(nextPlan)
        setCountries(nextCountries)
        if (nextPlan?.countryName) void getTourPlace(nextPlan.countryName).then((nextPlaces) => { if (isMounted) setPlaces(nextPlaces) }).catch(() => undefined)
      })
      .finally(() => { if (isMounted) setIsLoading(false) })
    return () => { isMounted = false }
  }, [])

  return { countries, isLoading: isLoading || isTripsLoading, places, plan, setPlan, trips }
}
...
function PlannerFlowPage({ step }: Props) {
  ...
  const continueTo = (next: string) => navigate(next)
  ...
  return (
    <AppShell>
      <S.Page>
        ...
        {step === 'list' ? <S.CardGrid>{trips.map((trip, index) => <S.TripCard key={trip.tripId ?? index} type="button" onClick={() => navigate(paths.plannerProgress)}><img src={trip.images?.[0] || figmaTripPlanning} alt="" /><strong>{trip.title || '여행 계획'}</strong><span>{dateLabel(trip.startDate)} – {dateLabel(trip.endDate)}</span><small>계획 중</small></S.TripCard>)}<S.AddCard type="button" onClick={() => continueTo(paths.plannerCreate)}>+ 새 여행 만들기</S.AddCard>{!isLoading && trips.length === 0 ? <S.Empty>아직 여행 계획이 없습니다.</S.Empty> : null}</S.CardGrid> : null}
        ...
        {step === 'group' ? <S.StepCard><S.StepNumber>02</S.StepNumber><h2>그룹 멤버를 확인하세요.</h2><S.MemberList><S.Member>나 <span>현재 사용자</span></S.Member><S.Member disabled>멤버 초대 API 미연동 <span>백엔드 계약 필요</span></S.Member></S.MemberList><S.ActionRow><PartTripButton type="button" $variant="secondary" onClick={() => navigate(paths.plannerCreate)}>이전</PartTripButton><PartTripButton type="button" onClick={() => navigate(paths.plannerDestination)}>다음</PartTripButton></S.ActionRow></S.StepCard> : null}

        {step === 'destination' ? <S.StepCard as="form" onSubmit={(event) => void saveDestination(event)}><S.StepNumber>03</S.StepNumber><S.FormGrid><S.Field><label htmlFor="planner-country">여행지</label><select id="planner-country" value={countryInfoId} onChange={(event) => { setCountryInfoId(event.target.value); const item = countries.find((country) => String(country.countryInfoId) === event.target.value); setCountryName(item?.countryName ?? ''); setCityName(item?.cityName ?? '') }}><option value="">여행지를 선택하세요</option>{countries.map((country) => <option key={country.countryInfoId ?? country.countryName} value={country.countryInfoId}>{country.cityName || country.countryName}</option>)}</select></S.Field><S.Field><label htmlFor="planner-city">도시</label><PartTripInput id="planner-city" value={cityName} onChange={(event) => setCityName(event.target.value)} placeholder="도시" /></S.Field><S.Field><label htmlFor="planner-start">시작일</label><PartTripInput id="planner-start" type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /></S.Field><S.Field><label htmlFor="planner-end">종료일</label><PartTripInput id="planner-end" type="date" value={endDate} onChange={(event) => setEndDate(event.target.value)} /></S.Field></S.FormGrid><S.ActionRow><PartTripButton type="button" $variant="secondary" onClick={() => navigate(paths.plannerGroup)}>이전</PartTripButton><PartTripButton type="submit" disabled={isSaving}>{isSaving ? '저장 중' : '여행 정보 저장'}</PartTripButton></S.ActionRow></S.StepCard> : null}

        {step === 'explore' ? <S.ExploreGrid>{['명소', '맛집', '카페'].map((category) => <S.CategoryCard key={category} type="button" onClick={() => navigate(paths.plannerVote)}><strong>{category}</strong><span>후보 장소를 조회하고 선택하기</span><b>›</b></S.CategoryCard>)}</S.ExploreGrid> : null}

        {step === 'vote' ? <S.VoteCard><S.Notice>장소 투표 API 계약이 없어 선택 상태만 화면에서 관리됩니다.</S.Notice>{places.map((placeItem, index) => <S.VoteRow key={`${placeItem.placeName}-${index}`}><div><small>후보 장소</small><strong>{placeItem.placeName || '장소'}</strong></div><S.CheckButton type="button" aria-pressed={selected.includes(index)} $selected={selected.includes(index)} onClick={() => setSelected((current) => current.includes(index) ? current.filter((item) => item !== index) : [...current, index])}>{selected.includes(index) ? '선택됨' : '선택'}</S.CheckButton></S.VoteRow>)}{places.length === 0 ? <S.Empty>연동된 장소 후보가 없습니다.</S.Empty> : null}<S.ActionRow><PartTripButton type="button" onClick={() => navigate(paths.plannerLineup)}>선택한 장소 정리 ({selected.length})</PartTripButton></S.ActionRow></S.VoteCard> : null}

        {step === 'lineup' ? <S.StepCard><S.StepNumber>05</S.StepNumber><h2>라인업 및 장바구니</h2>{selectedPlaces.length ? <S.SelectedList>{selectedPlaces.map((item) => <li key={item.placeName}>{item.placeName}</li>)}</S.SelectedList> : <S.Empty>선택한 장소가 없습니다. 투표 화면에서 장소를 선택하세요.</S.Empty>}<S.ActionRow><PartTripButton type="button" $variant="secondary" onClick={() => navigate(paths.plannerVote)}>장소 더 찾기</PartTripButton><PartTripButton type="button" onClick={() => navigate(paths.plannerFinal)}>최종 계획 확인</PartTripButton></S.ActionRow></S.StepCard> : null}

        {step === 'progress' ? <S.ProgressLayout><S.StepCard><S.StepNumber>진행 중</S.StepNumber><h2>{plan?.cityName || '여행 계획'} 계획</h2><p>{dateLabel(plan?.startDate)} – {dateLabel(plan?.endDate)}</p><S.ProgressTrack><S.ProgressBar $progress={plan ? 60 : 0} /></S.ProgressTrack><S.StatusPill>여행지 정보 저장 완료</S.StatusPill><S.StatusPill $warning>플래너 투표 API 미연동</S.StatusPill><S.ActionRow><PartTripButton type="button" onClick={() => navigate(paths.plannerExplore)}>장소 찾기</PartTripButton><PartTripButton type="button" $variant="secondary" onClick={() => navigate(paths.plannerFinal)}>최종 계획</PartTripButton></S.ActionRow></S.StepCard><S.SideSummary><h2>해야 할 일</h2><p>장소 후보를 확인하고 투표를 이어가세요.</p><button type="button" onClick={() => navigate(paths.plannerVote)}>투표 화면 열기 ›</button></S.SideSummary></S.ProgressLayout> : null}

        {step === 'final' ? <S.ProgressLayout><S.StepCard><S.StepNumber>확정 전</S.StepNumber><h2>최종 계획 확인</h2><p>{plan ? `${plan.cityName || plan.countryName} · ${dateLabel(plan.startDate)} – ${dateLabel(plan.endDate)}` : '저장된 여행 정보가 없습니다.'}</p><S.Notice>여행 계획 저장 API가 제공되면 이 단계에서 확정 동작을 연결합니다.</S.Notice><S.ActionRow><PartTripButton type="button" onClick={() => navigate(paths.plannerProgress)}>진행 상태로 돌아가기</PartTripButton><PartTripButton type="button" $variant="secondary" onClick={() => navigate(paths.tripCards)}>여행 카드 보기</PartTripButton></S.ActionRow></S.StepCard><S.SideSummary><h2>선택 장소</h2><p>{places.length ? `${places.length}곳의 후보 장소` : '장소 후보 없음'}</p></S.SideSummary></S.ProgressLayout> : null}

        {step === 'place' ? <S.PlaceLayout>{place ? <><S.PlaceImage src={place.imageUrl || figmaTripPlanning} alt="" /><S.StepCard><S.Badge>추천 장소</S.Badge><h2>{place.placeName}</h2><p>{place.description || '장소 설명이 없습니다.'}</p><PartTripButton type="button" onClick={() => navigate(paths.plannerVote)}>투표 후보에 추가</PartTripButton></S.StepCard></> : <S.Empty>장소 정보를 찾을 수 없습니다.</S.Empty>}</S.PlaceLayout> : null}
      </S.Page>
    </AppShell>
  )
}
