...
function Header({
  ...
  return <S.Header><div><S.Title>{title}</S.Title><S.Subtitle>{subtitle}</S.Subtitle></div>{showNewTrip ? <PartTripButton type="button" onClick={onNewTrip}>새 여행</PartTripButton> : null}</S.Header>
}
...
function PlannerFlowPage({ step }: Props) {
  ...
  const selectedPlanner = availablePlanners.find((planner) => planner.plannerId === selectedPlannerId) ?? availablePlanners[0]
  ...
  const voteOptions = activeVote?.options?.length
    ? activeVote.options.map((option) => ({ address: option.address, imageUrl: option.imageUrl, optionId: option.optionId, placeName: option.placeName, rating: option.rating, voteCount: option.voteCount }))
    : places.map((item) => ({ address: item.address, imageUrl: item.imageUrl, optionId: item.tourPlaceId, placeName: item.placeName, rating: item.rating, voteCount: undefined }))
  ...
  return (
    <AppShell>
      <S.Page>
        ...
        {step === 'list' ? (
          <>
            ...
            <S.PlannerListLayout>
              ...
              {selectedPlanner ? <S.SelectedPlanPanel><img src={selectedPlanner.status && statusKey(selectedPlanner.status) === 'completed' ? figmaTripPlanning : figmaHomeHero} alt="" /><div><h2>{selectedPlanner.title || `${selectedPlanner.cityName || selectedPlanner.countryName || '여행'} 여행`}</h2><p>멤버 {selectedPlanner.joinedMemberCount ?? selectedPlanner.memberCount ?? '-'}명 · 투표 {confirmedCount || '-'}</p><small>여행 준비 진행률</small><S.ProgressTrack><S.ProgressBar $progress={selectedPlanner.joinedMemberCount && selectedPlanner.memberCount ? Math.round((selectedPlanner.joinedMemberCount / selectedPlanner.memberCount) * 100) : 40} /></S.ProgressTrack><S.MemberAvatars><S.Avatar>MS</S.Avatar><S.Avatar>JH</S.Avatar><S.Avatar>AY</S.Avatar></S.MemberAvatars><PartTripButton type="button" onClick={() => handleSelectPlanner(selectedPlanner.plannerId)}>상세 단계 보기</PartTripButton></div></S.SelectedPlanPanel> : null}
            </S.PlannerListLayout>
          </>
        ) : null}
        ...
        {step === 'group' ? <S.TwoColumn><S.StepCard as="form" onSubmit={saveGroupSettings}><S.SectionTitle>여행 유형</S.SectionTitle><S.SegmentRow><S.SegmentButton type="button" $active={isSolo} onClick={() => setIsSolo(true)}>혼자 여행</S.SegmentButton><S.SegmentButton type="button" $active={!isSolo} onClick={() => setIsSolo(false)}>함께 여행</S.SegmentButton></S.SegmentRow><S.StepField><label htmlFor="planner-member-count">인원 수 (본인 포함)</label><S.Stepper><span>{isSolo ? 1 : memberCount}</span><button type="button" aria-label="인원 줄이기" onClick={() => setMemberCount(String(Math.max(1, Number(memberCount) - 1)))} disabled={isSolo}>−</button><button type="button" aria-label="인원 늘리기" onClick={() => setMemberCount(String(Math.min(30, Number(memberCount) + 1)))} disabled={isSolo}>+</button></S.Stepper></S.StepField><S.SectionTitle>참여 멤버</S.SectionTitle><S.MemberList><S.MemberRow><S.Avatar>{currentUserInitial}</S.Avatar><S.MemberDetails><strong>{currentUserName}</strong><span>호스트</span></S.MemberDetails><S.MemberState>호스트</S.MemberState></S.MemberRow>{members.map((member, index) => <S.MemberRow key={`${member.userId ?? member.nickName}-${index}`}><S.Avatar>{(member.nickName || member.userId || '멤버').slice(0, 2).toUpperCase()}</S.Avatar><S.MemberDetails><strong>{member.nickName || member.userId || '멤버'}</strong><span>멤버</span></S.MemberDetails><S.MemberState>{member.role || '참여 중'}</S.MemberState></S.MemberRow>)}</S.MemberList><S.ActionRow><PartTripButton type="button" $variant="secondary" onClick={() => navigate({ to: paths.plannerCreate })}>이전</PartTripButton><PartTripButton type="submit">다음</PartTripButton></S.ActionRow></S.StepCard><S.InvitePanel><S.SectionTitle>함께할 사람</S.SectionTitle><p>링크로 친구를 초대하세요.</p><PartTripButton type="button" $variant="secondary" disabled={!plannerInviteCode} onClick={() => void navigator.clipboard?.writeText(plannerInviteCode)}>+ 링크로 초대하기</PartTripButton></S.InvitePanel></S.TwoColumn> : null}
        ...
        {step === 'explore' ? <><S.CategoryChips aria-label="장소 카테고리">{categories.map((category) => <S.CategoryChip key={category} type="button" $active={voteCategory === category} onClick={() => { setVoteCategory(category); setSelected([]) }}>{category}</S.CategoryChip>)}</S.CategoryChips><S.PlaceBody><S.PlaceListPanel><h2>{voteCategory} {places.length}곳 · 클릭해서 담기</h2>{places.map((item, index) => <S.PlaceRow key={`${item.placeName}-${index}`}><S.PlaceThumb $imageUrl={item.imageUrl} /><S.PlaceDetails><strong>{item.placeName || '장소'}</strong><span>★ {item.rating ?? '-'} · {item.address || '상세 정보'}</span></S.PlaceDetails><S.PlaceAction type="button" $active={selected.includes(index)} onClick={() => setSelected((current) => current.includes(index) ? current.filter((value) => value !== index) : [...current, index])}>{selected.includes(index) ? '✓ 담김' : '담기'}</S.PlaceAction></S.PlaceRow>)}{places.length === 0 ? <S.Empty>연동된 장소 후보가 없습니다.</S.Empty> : null}</S.PlaceListPanel><S.SelectedPanel><h2>선택한 장소 {selected.length}</h2>{selectedPlaces.map(({ index, item }) => <S.SelectedRow key={`${item.placeName}-${index}`}><span>{item.placeName || '장소'}</span><button type="button" aria-label={`${item.placeName || '장소'} 선택 취소`} onClick={() => handleRemoveFromLineup(index)}>×</button></S.SelectedRow>)}{selected.length === 0 ? <S.Empty>장소를 담아보세요.</S.Empty> : null}<S.SelectionGuidance><strong>선택한 장소는 투표 후보로 사용돼요</strong><span>후보를 더 담거나 투표를 시작할 수 있어요.</span></S.SelectionGuidance><S.PanelActions><PartTripButton type="button" disabled={createVoteMutation.isPending || selected.length === 0} onClick={() => void handleCreateVote()}>{createVoteMutation.isPending ? '투표 준비 중' : '투표 시작하기'}</PartTripButton><PartTripButton type="button" $variant="secondary" onClick={() => flowNavigate({ to: paths.plannerExplore })}>장소 더 찾기</PartTripButton></S.PanelActions></S.SelectedPanel></S.PlaceBody></> : null}

        {step === 'vote' ? <><S.CategoryChips aria-label="투표 카테고리">{categories.map((category) => <S.CategoryChip key={category} type="button" $active={voteCategory === category} onClick={() => { setVoteCategory(category); setSelected([]) }}>{category}</S.CategoryChip>)}</S.CategoryChips><S.VoteBody><S.CandidatePanel><h2>{voteCategory} 후보 {voteOptions.length}곳 · 한 곳을 선택하세요</h2>{voteOptions.map((option, index) => <S.CandidateRow key={`${option.placeName}-${index}`}><S.PlaceThumb $imageUrl={option.imageUrl} /><S.PlaceDetails><strong>{option.placeName || '장소'}</strong><span>{option.address || '상세 정보'}</span></S.PlaceDetails><S.VoteMeta $selected={selected.includes(index)}><span>{option.voteCount ?? 0}표</span><button type="button" aria-pressed={selected.includes(index)} disabled={castBallotMutation.isPending} onClick={() => { setSelected([index]); void handleCastBallot(option.optionId) }}>{selected.includes(index) ? '투표함' : '투표'}</button></S.VoteMeta></S.CandidateRow>)}{voteOptions.length === 0 ? <S.Empty>투표 후보가 없습니다.</S.Empty> : null}<S.PanelActions><PartTripButton type="button" onClick={nextVoteCategory}>다음 카테고리</PartTripButton></S.PanelActions></S.CandidatePanel></S.VoteBody></> : null}
