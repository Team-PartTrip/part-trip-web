...
export function usePlannerCandidateFlow({ canManagePlanner, data, navigate, mutations, placeId, state }: Props) {
  ...
  const { canManageCandidates: canManageOpenCandidates, candidateManagementError } = getCandidateManagementState(votes, votesLoading, votesError)
  const canManageCandidates = canManagePlanner && canManageOpenCandidates
  ...
  const handleAddPlaceCandidate = async () => {
    ...
    if (!canManagePlanner) {
      setErrorMessage('플래너 소유자만 투표 후보를 추가할 수 있습니다.')
      return
    }
    if (candidateManagementError) {
      setErrorMessage(candidateManagementError)
