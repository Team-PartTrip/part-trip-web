...
function duration(trips: TripPlanResponseDto[]) {
  return trips.reduce((sum, trip) => trip.startDate && trip.endDate ? sum + Math.max(1, Math.round((Date.parse(trip.endDate) - Date.parse(trip.startDate)) / 86400000) + 1) : sum, 0)
}
...
export function ProfileInsightPage({ kind }: { kind: ProfileInsightKind }) {
  ...
  return (
    <AppShell>
      <S.Page>
        <S.Header><div><S.Title>{title}</S.Title><S.Subtitle>{subtitle}</S.Subtitle></div><S.BackButton type="button" onClick={() => navigate(paths.profile)}>마이페이지</S.BackButton></S.Header>
        ...
        {!isLoading && !errorMessage && kind === 'achievements' ? <S.AchievementGrid><S.Achievement><strong>{countries.length}</strong><span>방문 국가</span></S.Achievement><S.Achievement><strong>{trips.length}</strong><span>여행 기록</span></S.Achievement><S.Achievement><strong>{duration(trips)}</strong><span>여행 일수</span></S.Achievement></S.AchievementGrid> : null}
      </S.Page>
    </AppShell>
  )
}
