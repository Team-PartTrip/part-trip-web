...
function TripCardsFlow({ mode }: CardPageProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      await importTrip(detail.tripId)
  ...
  return (
    <AppShell>
      <S.Page>
        ...
        {mode === 'create' && !isLoading ? <S.SelectList>{mine.map((trip, index) => <S.SelectRow key={trip.tripId ?? index}><div><strong>{trip.title || '여행 기록'}</strong><span>{trip.cityName || trip.countryName || '여행지'}</span></div><PartTripButton type="button" onClick={() => void handleShare(trip.tripId)}>공유하기</PartTripButton></S.SelectRow>)}{mine.length === 0 ? <S.Empty>공유할 내 여행이 없습니다.</S.Empty> : null}</S.SelectList> : null}
