...
export function RecordDetailPage() {
  ...
  return (
    <AppShell>
      <S.Content>
        <S.TopBar><div><h1>기록 상세</h1><p>한 장소에서 남긴 사진과 메모를 확인하세요.</p></div></S.TopBar>
        {errorMessage || hasRecordError ? <S.ErrorMessage role="alert">{errorMessage || '여행 기록을 불러오지 못했습니다.'}</S.ErrorMessage> : null}
        {isLoading ? <S.StateCard>여행 기록을 불러오고 있습니다.</S.StateCard> : record ? isEditing ? <S.EditForm onSubmit={(event) => void handleUpdate(event)}><h1>여행 기록 수정</h1><label>제목<input value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} maxLength={60} /></label><S.DateFields><label>시작일<input type="date" value={draft.startDate} onChange={(event) => setDraft((current) => ({ ...current, startDate: event.target.value }))} /></label><label>종료일<input type="date" value={draft.endDate} onChange={(event) => setDraft((current) => ({ ...current, endDate: event.target.value }))} /></label></S.DateFields><label>여행 메모<textarea value={draft.content} onChange={(event) => setDraft((current) => ({ ...current, content: event.target.value }))} maxLength={1000} /></label><div><button type="button" onClick={() => setIsEditing(false)}>취소</button><button type="submit" disabled={updateMutation.isPending}>{updateMutation.isPending ? '저장 중' : '저장'}</button></div></S.EditForm> : <S.DetailBody><S.RecordPhoto><img src={record.images?.[0] || figmaRecordDetail} alt={`${record.cityName || record.countryName || '여행'} 기록 사진`} /></S.RecordPhoto><S.RecordDetailCard><h1>{record.title || record.cityName || 'Dotonbori'}</h1><p>{[record.cityName || record.countryName, formatDate(record.startDate)].filter(Boolean).join(' · ')}</p><S.Badge>여행 기록</S.Badge><S.RecordDescription>{record.content || '작성된 여행 메모가 없습니다.'}</S.RecordDescription><button type="button" onClick={startEditing}>기록 편집</button><button type="button" onClick={() => navigate({ params: { recordId }, to: '/record/$recordId/edit' })}>코멘트 수정</button><button type="button" onClick={() => void handleDelete()} disabled={deleteMutation.isPending}>기록 삭제</button></S.RecordDetailCard></S.DetailBody> : <S.StateCard><h1>여행 기록을 찾을 수 없습니다.</h1><p>목록에서 다른 기록을 선택해주세요.</p><button type="button" onClick={() => navigate({ to: paths.record })}>목록으로 돌아가기</button></S.StateCard>}
      </S.Content>
    </AppShell>
  )
}
