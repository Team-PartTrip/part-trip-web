...
export function RecordCommentEditPage() {
  ...
  const isSaveDisabled = updateMutation.isPending || editableEntry?.entryId == null
  ...
  return (
    <AppShell>
      <S.Content>
        <S.TopBar $comment>
          <div><h1>코멘트 작성</h1></div>
          <div>
            <button type="submit" form="record-comment-form" disabled={isSaveDisabled}>저장</button>
          </div>
        </S.TopBar>
        {isLoading ? (
          <S.LoadingLayout aria-busy="true" aria-label="코멘트 기록 로딩 중">
            <S.LoadingHeader />
        ...
        {record ? (
          <S.CommentEditLayout>
            <S.CommentPhoto>
              <img src={editableEntry?.imageUrl || record.images?.[0] || figmaRecordMapPhoto} alt="여행 사진" />
              <h2>{placeTitle}</h2>
              <span>{photoDate ? formatDate(photoDate) : '촬영일 정보 없음'}</span>
            </S.CommentPhoto>
            <S.CommentForm id="record-comment-form" onSubmit={(event) => void handleSubmit(event)}>
              <h2>코멘트</h2>
              <textarea
                value={contentValue}
                ...
                maxLength={100}
                disabled={editableEntry?.entryId == null}
              />
              <p>{contentValue.length} / 100</p>
              <button type="submit" disabled={isSaveDisabled}>
                {updateMutation.isPending ? '저장 중' : '코멘트 저장'}
              </button>
        ...
        {!isLoading && !isError && !record ? (
          <S.StateCard>
            <p>기록을 찾을 수 없습니다.</p>
            <button type="button" onClick={() => navigate({ to: paths.record })}>목록으로</button>
          </S.StateCard>
        ) : null}
      </S.Content>
    </AppShell>
