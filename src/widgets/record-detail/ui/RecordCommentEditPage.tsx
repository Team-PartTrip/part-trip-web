  ...
  return (
    <AppShell>
      <S.Content>
        ...
        {record ? <S.CommentEditLayout><S.CommentPhoto><img src={editableEntry?.imageUrl || record.images?.[0] || figmaRecordMapPhoto} alt="여행 사진" /><h2>{placeTitle}</h2><span>{photoDate ? formatDate(photoDate) : '촬영일 정보 없음'}</span></S.CommentPhoto><S.CommentForm id="record-comment-form" onSubmit={(event) => void handleSubmit(event)}><h2>코멘트</h2><textarea value={contentValue} onChange={(event) => setContent(event.target.value)} placeholder="이 사진에 대해 짧게 남겨보세요." maxLength={100} disabled={editableEntry?.entryId == null} /><p>{contentValue.length} / 100</p><button type="submit" disabled={updateMutation.isPending || editableEntry?.entryId == null}>{updateMutation.isPending ? '저장 중' : '코멘트 저장'}</button>{message ? <small role="alert">{message}</small> : null}</S.CommentForm></S.CommentEditLayout> : null}
