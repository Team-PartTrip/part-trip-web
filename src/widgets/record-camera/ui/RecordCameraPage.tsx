...
export function RecordCameraPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      navigate(createRecordCameraDetailPath(result.imageId))
    } catch {
  ...
  return (
    <AppShell>
      <S.Page>
        <S.Header><div><S.Title>촬영 기록 분석</S.Title><S.Subtitle>사진을 올리면 장소와 여행 정보를 분석합니다.</S.Subtitle></div><PartTripButton type="button" $variant="secondary" onClick={() => navigate(paths.record)}>여행 기록</PartTripButton></S.Header>
...
export function RecordCameraDetailPage() {
  ...
  const [result, setResult] = useState<PhotoAnalysisResponseDto | null>(null)
  const [errorMessage, setErrorMessage] = useState(isValidImageId ? '' : '잘못된 분석 결과입니다.')
  const [isLoading, setIsLoading] = useState(isValidImageId)

  useEffect(() => {
    if (!isValidImageId) return
    let isMounted = true
    void getGuideCameraResult(parsedImageId)
      .then((data) => { if (isMounted) setResult(data) })
      .catch(() => { if (isMounted) setErrorMessage('분석 결과를 불러오지 못했습니다.') })
      .finally(() => { if (isMounted) setIsLoading(false) })
    return () => { isMounted = false }
  }, [imageId, isValidImageId, parsedImageId])

  return (
    <AppShell>
      <S.Page>
        <S.Header><div><S.Title>촬영 기록 상세</S.Title><S.Subtitle>분석된 장소와 설명을 확인하세요.</S.Subtitle></div><PartTripButton type="button" $variant="secondary" onClick={() => navigate(paths.recordCamera)}>사진 다시 올리기</PartTripButton></S.Header>
        {isLoading ? <S.State aria-busy="true">분석 결과를 불러오는 중입니다.</S.State> : null}
        {errorMessage ? <S.Error role="alert">{errorMessage}</S.Error> : null}
        {result ? <S.DetailCard><S.Badge>분석 완료</S.Badge><h2>{result.title || result.designation || '분석된 장소'}</h2><p>{result.era || result.current_status || ''}</p><S.DetailGrid><div><small>개요</small><span>{result.overview || '개요가 없습니다.'}</span></div><div><small>배경</small><span>{result.background || '배경 정보가 없습니다.'}</span></div><div><small>특징</small><span>{result.features || '특징 정보가 없습니다.'}</span></div><div><small>출처</small><span>{result.sourceName || '출처 정보가 없습니다.'}</span></div></S.DetailGrid><PartTripButton type="button" disabled={result.photoId == null} onClick={() => navigate(createRecordCameraWritePath(Number(imageId)))}>코멘트 작성</PartTripButton></S.DetailCard> : null}
      </S.Page>
    </AppShell>
  )
}
...
export function RecordCameraWritePage() {
  ...
  useEffect(() => {
    ...
    void getGuideCameraResult(parsedImageId)
      .then((data) => {
        if (!isMounted) return
        setResult(data)
        setTitle(data.commTitle || data.title || '')
        setContent(data.commContent || '')
        if (data.photoDate) setPhotoDate(data.photoDate)
      })
      .catch(() => { if (isMounted) setErrorMessage('분석 결과를 불러오지 못했습니다.') })
      .finally(() => { if (isMounted) setIsLoading(false) })
    return () => { isMounted = false }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    if (result?.photoId == null || !title.trim() || !content.trim() || !photoDate) {
  ...
  return (
    <AppShell>
      <S.Page>
        <S.Header><div><S.Title>{saved ? '기록 저장 완료' : '코멘트 작성'}</S.Title><S.Subtitle>{saved ? '촬영 기록이 저장되었습니다.' : '분석 결과에 나만의 코멘트를 남겨보세요.'}</S.Subtitle></div><PartTripButton type="button" $variant="secondary" onClick={() => navigate(createRecordCameraDetailPath(Number(imageId)))}>상세로 돌아가기</PartTripButton></S.Header>
        {isLoading ? <S.State aria-busy="true">기록 정보를 준비하는 중입니다.</S.State> : null}
        {errorMessage ? <S.Error role="alert">{errorMessage}</S.Error> : null}
        {saved ? <S.DetailCard><S.Badge>저장 완료</S.Badge><h2>{saved.commTitle || title}</h2><p>{saved.commContent || content}</p><PartTripButton type="button" onClick={() => navigate(paths.record)}>여행 기록으로 이동</PartTripButton></S.DetailCard> : <S.Form onSubmit={(event) => void handleSubmit(event)}><S.Field><label htmlFor="camera-title">제목</label><PartTripInput id="camera-title" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="기록 제목" /></S.Field><S.Field><label htmlFor="camera-date">촬영일</label><PartTripInput id="camera-date" type="date" value={photoDate} onChange={(event) => setPhotoDate(event.target.value)} /></S.Field><S.Field><label htmlFor="camera-content">코멘트</label><PartTripTextarea id="camera-content" value={content} onChange={(event) => setContent(event.target.value)} placeholder="이 장소에서의 기억을 남겨보세요." /></S.Field><PartTripButton type="submit" disabled={isSaving || isLoading}>{isSaving ? '저장 중' : '기록 저장'}</PartTripButton></S.Form>}
      </S.Page>
    </AppShell>
  )
}
