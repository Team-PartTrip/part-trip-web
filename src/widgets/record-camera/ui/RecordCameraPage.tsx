...
export function RecordCameraPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const parsedTravelId = Number(selectedTravelId)
    if (!isPositiveSafeInteger(parsedTravelId) || !imageFile || latitude == null || longitude == null) {
  ...
  return (
    <AppShell>
      <S.Page>
        ...
        {isLoading ? <S.LoadingCard aria-busy="true" aria-label="여행 기록 로딩 중" /> : <S.Form onSubmit={(event) => void handleSubmit(event)}>
...
export function RecordCameraDetailPage() {
  ...
  const isValidImageId = isPositiveSafeInteger(parsedImageId)
  ...
  return (
    <AppShell>
      <S.Page>
        ...
        {isLoading ? <S.LoadingCard aria-busy="true" aria-label="기록 정보 로딩 중" /> : saved ? <S.DetailCard><S.Badge>저장 완료</S.Badge><h2>{saved.commTitle || titleValue}</h2><p>{saved.commContent || contentValue}</p><PartTripButton type="button" onClick={() => navigate({ to: paths.record })}>여행 기록으로 이동</PartTripButton></S.DetailCard> : result?.photoId != null ? <S.Form onSubmit={(event) => void handleSubmit(event)}><S.Field><label htmlFor="camera-title">제목</label><PartTripInput id="camera-title" value={titleValue} onChange={(event) => setTitle(event.target.value)} placeholder="기록 제목" /></S.Field><S.Field><label htmlFor="camera-date">촬영일</label><PartTripInput id="camera-date" type="date" value={photoDateValue} onChange={(event) => setPhotoDate(event.target.value)} /></S.Field><S.Field><label htmlFor="camera-content">코멘트</label><PartTripTextarea id="camera-content" value={contentValue} onChange={(event) => setContent(event.target.value)} placeholder="이 장소에서의 기억을 남겨보세요." /></S.Field><PartTripButton type="submit" disabled={isSaving || isLoading}>{isSaving ? '저장 중' : '기록 저장'}</PartTripButton></S.Form> : <S.State>작성 가능한 분석 결과가 없습니다.</S.State>}
      </S.Page>
    </AppShell>
  )
}
