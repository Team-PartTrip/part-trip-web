...
export function useChangePasswordFlow() {
  ...
  return {
    ...
    isSendingCode,
    isVerificationSubmitting: verificationForm.formState.isSubmitting,
    message,
    newPasswordConfirmField,
    newPasswordField,
    onBack: () => navigate({ to: paths.login }),
    onPasswordSubmit: passwordForm.handleSubmit(handlePasswordSubmit, handlePasswordInvalid),
    onSendCode: () => { void handleSendVerificationCode() },
    onVerificationSubmit: verificationForm.handleSubmit(handleVerificationSubmit, handleVerificationInvalid),
    step,
